...
public class ProductsPage extends AbstractPage {
    ...
    public ProductsPage filterBrandsByKeyword (final String keyword) {
        new TextInput(driver, brandSearchBlock.findElement(By.xpath("//input[@name='searchline']"))).safeSendKeysAndWaitForUpdate(brandSearchBlock, 15, keyword);
        return new ProductsPage(driver);
    }

    public ProductsPage selectBrand (final String brandName) {
        new CheckBoxFilterItem(driver, brandSearchBlock.findElement(By.xpath(String.format("//label[contains(text(),'%s')]/parent::a", brandName)))
        ).setCheckedThenWaitForUpdate(true, brandSearchBlock, 15);
        return new ProductsPage(driver);
    }

    private ProductsPage sortProductsByValue(String orderValue) {
        Select sortSelect = new Select(driver.findElement(By.xpath("//rz-sort/select")));
        AbstractElement.performAndWaitForUpdate(driver, () -> sortSelect.selectByValue(orderValue), firstProductImageButton, 15);
        return new ProductsPage(driver);
    }
